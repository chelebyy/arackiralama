...
function buildTargetUrl(req: NextRequest, pathSegments: string[]) {
  ...
  return `${base}/api/v1/${pathSegments.join("/")}${search}`;
}
...
async function forward(
  ...
  if (!path || path.length === 0) {
    return makeErrorResponse(400, "Missing public API path");
  }
  ...
  const upstream = await fetch(buildTargetUrl(req, path), init);
