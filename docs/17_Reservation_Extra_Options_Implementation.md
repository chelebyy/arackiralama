...
- PR #386 review follow-up maps invalid legacy quantities to HTTP 400 on paid and unpaid endpoints, preserves percentage/fixed campaign semantics after catalog-backed legacy-extra adaptation, and prevents changed selections from being automatically resubmitted after a `409`. Focused backend tests passed 34/34, the full backend unit suite passed 733/733, TypeScript passed, Step 4 Vitest passed 15/15, the rebuilt Docker production web passed the focused conflict Chromium scenario 1/1, and PR CI was green before this follow-up push. Aikido, deployment/rollback, and production observation remain open.
