...
### 9.6 Local Revalidation and Public-Code Length Hardening (17 July 2026)
...
The current Dokploy deployment is a development/staging environment, not the final production VPS. No controlled staging fixture was created. The overlength fix and controlled staging payload/owner-cancellation matrix remain deployment acceptance work after merge; final-production evidence remains a later environment gate.
...
| WP2 | Locally revalidated; input hardening pending deployment | Allowlisted public DTO, public frontend client paths limited to that DTO, strict rate limit, no-store, anonymous cancellation removal, owner/admin paths preserved; fresh local Docker Chromium confirms the exact public allowlist through all five locale pages and proves anonymous/non-owner no-write plus owner cancellation; 807/807 unit and 53/53 integration tests pass; the 24-character public-code limit is now enforced before EF and lengths 24/25/128 return uniform local `404 + no-store` | Merge and deploy the length guard, then run the controlled reservation payload and authenticated owner/non-owner matrix in Dokploy development/staging. The final production VPS does not yet exist and remains a separate acceptance gate |
