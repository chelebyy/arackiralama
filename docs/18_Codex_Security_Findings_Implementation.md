...
- Dependabot cannot merge without the approved review policy;
...
7. What branch-protection policy must Dependabot PRs satisfy?
...
| WP2 | Locally acceptance-proven | Allowlisted public DTO, strict rate limit, no-store, anonymous cancellation removal, owner/admin paths preserved; production-like Chromium confirms the exact public allowlist through all five locale pages and proves anonymous/non-owner no-write plus owner cancellation | Deployment rerun and focused final security review |
...
| WP4 | Partial | Generated artifact ignore/removal and auto-merge workflow removal; Gitleaks GitHub Actions workflow scans the working tree and full Git history, uses fingerprint-scoped historical fixture ignores, and retains only a seven-day sanitized metadata artifact | Provider credential rotation/access-log evidence; first remote workflow run after push; GitHub branch protection and test Dependabot PR proof |
| WP5 | Blocked | Backend build and full backend suites pass; frontend lint/typecheck/test/build pass; the production payment `ValidateOnStart` host-start and production-like Docker startup/healthy-state matrices pass locally; local Docker disabled-payment HTTP/no-write proof passes; account-claim and public reservation/cancellation Chromium proofs pass across all five locales | Operational credential/GitHub evidence, deployment rerun, and focused final security review; provider sandbox evidence is deferred until payments are introduced |

### 18.1 Fresh Automated Evidence

- Backend build: 0 warnings, 0 errors.
- Focused payment configuration tests: 12/12 passed. The real host-start path rejects missing, Mock, unknown, sandbox, and incomplete Production configurations; a fully configured Iyzico Production host and an intentional Development Mock host start successfully.
