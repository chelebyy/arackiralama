...
### 9.6 Local Revalidation, Public-Code Length Hardening, and Staging Acceptance (17-18 July 2026)

The current WP2 rerun used the real local Docker API/web/PostgreSQL boundary and the existing self-cleaning Chromium fixture. The five localized confirmation pages returned the exact 10-field allowlist with `no-store`; anonymous and foreign-customer cancellation left `status`, `xmin`, and `updated_at` unchanged; owner cancellation returned `200` and persisted `Cancelled`. Final marker counts were `customers=0`, `reservations=0`, and `jobs=0`.

Read-only Dokploy preflight showed that normal nonexistent codes return uniform `404 + no-store`, while an input longer than the 24-character database limit returned a generic `500`. The scoped hardening branch now centralizes `Reservation.PublicCodeMaxLength`, rejects overlength input before the repository with the existing not-found envelope, and covers both the 25-character negative and 24-character positive boundary. Local HTTP checks for lengths 24, 25, and 128 now return `404 + no-store`; only the supported-length miss reaches EF.

PR #415 was squash-merged to `main` as `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145`, and the operator confirmed that exact commit as the active Dokploy deployment. After the VPS was stabilized on 18 July, the controlled matrix passed against the development/staging stack: all five localized confirmation pages returned the exact 10-field allowlist with `no-store` and none of the seeded internal/PII values; anonymous and foreign-customer cancellation returned `404` without changing the reservation `status|xmin|updated_at` fingerprint; owner cancellation returned `200` and persisted `Cancelled`; and direct deployed-API lookups at lengths 24, 25, and 128 returned `404 + no-store`. Fixture cleanup proved zero test-owned customers, reservations, auth sessions, background jobs, and audit logs (`0|0|0|0|0`). The five application containers remained healthy and PostgreSQL accepted connections after the run. This closes WP2 development/staging acceptance only; final-production VPS evidence remains a separate environment gate.
...
## 18. Implementation Status Snapshot (18 July 2026)
...
| WP1 | Public membership entry points disabled; local and development/staging deployed-public acceptance complete | The verified token implementation remains retained but unreachable through public HTTP entry points. The public header no longer renders the staff login action even when managed settings contain it; the customer login page no longer links to registration; registration and localized account-claim pages return `404`; frontend proxy and backend registration/claim routes return empty `404` responses before body parsing, controller execution, database writes, or claim-email job creation. Existing customer login remains available by direct route. Rebuilt local API/web images and the revised Chromium harness proved both backend/frontend `404`, all five localized claim pages disabled, no homepage login link, and no database/job side effects. PR #413 merged as `fb7ca83e01599556ea9b06d24d9c570a4d0a111b`; the exact commit passed post-merge CI/security + GHCR publication and operator-triggered Dokploy development/staging HTTP + Chromium acceptance. The 18 July follow-up added direct backend variants, container/log inspection, and unchanged staging DB/job fingerprints | Final-production internal-backend, container/log, and DB/job evidence remains a separate gate. Future reservation-notification email remains a separate undecided capability and must not reuse the disabled membership onboarding path implicitly |
| WP2 | Implementation, local acceptance, and Dokploy development/staging acceptance complete | Allowlisted public DTO, public frontend client paths limited to that DTO, strict rate limit, no-store, anonymous cancellation removal, owner/admin paths preserved; 807/807 unit and 53/53 integration tests pass; the 24-character public-code limit is enforced before EF; PR #415 merged as `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145`; the controlled staging Chromium/API/PostgreSQL matrix proved the exact five-locale allowlist, anonymous/non-owner no-write fingerprints, owner cancellation, 24/25/128 not-found guards, healthy services, and `0|0|0|0|0` fixture cleanup | Final-production VPS acceptance and revalidation of the remaining original attack paths remain separate. Do not generalize this focused staging result into release readiness |
...
| WP4 | Complete for the tracked repository-control, dependency-remediation, alert-reconciliation, and Dependabot-lifecycle scope | Generated artifact ignore/removal and auto-merge workflow removal; Gitleaks scans the working tree and full Git history and passed on PR #402 and post-merge `main`; the Resend and Upstash-shaped scanner candidates are not applicable for rotation; active ruleset `Protect main - solo developer` requires a pull request, resolved threads, a current branch, seven strict checks, squash merge, and blocks deletion/non-fast-forward updates without bypass actors; PR #405 merged the five patched dependency versions as `479317b` after Node 22 CI, all checks, and exact-head Codex review; GitHub marked all 11 original alert records fixed at `2026-07-15T15:27:57Z`-`2026-07-15T15:27:58Z` without dismissal; post-ruleset Dependabot PR #422 completed the required label, current-branch, exact-head, required/advisory-check, zero-thread, manual-decision, squash-merge, and post-merge `main` lifecycle as `134c6c888ff510c4eb1adfab1e41ebc0c5d83793` | Continue applying the same manual decision and evidence requirements to future Dependabot PRs; this closure does not imply production or whole-repository security acceptance |
| WP5 | Blocked | Backend build and full backend suites pass; frontend lint/typecheck/test/build pass; the production payment `ValidateOnStart` host-start and production-like Docker startup/healthy-state matrices pass locally; local Docker disabled-payment HTTP/no-write proof passes; account-claim and public reservation/cancellation Chromium proofs pass across all five locales; the focused final validation used application-under-test `202074f`, and the corrected committed claim harness re-run at `9420446` passed against the same Docker stack; five original findings are suppressed, both provider-shaped scanner candidates are not actionable, payment integrity remains deferred, and the development/staging Disabled-mode deployment/public acceptance gate passes; PR #405 dependency remediation, post-merge `main` checks, patched SBOM, and all 11 fixed alert records are evidenced; the post-ruleset Dependabot lifecycle completed through PR #422 and merge commit `134c6c888ff510c4eb1adfab1e41ebc0c5d83793`; the selected no-membership closure path passed local validation, merged through PR #413, and passed exact-commit Dokploy development/staging deployed-public acceptance at `fb7ca83e01599556ea9b06d24d9c570a4d0a111b`; WP2 passed its exact-commit Dokploy development/staging matrix at `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145` | Final-production revalidation of the remaining original attack paths remains separate; real payment-provider implementation/sandbox evidence remains deferred until payments are introduced |
...
- 18 July WP2 Dokploy development/staging acceptance: the operator-confirmed deployed commit was `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145`. A temporary, self-cleaning Playwright harness opened the verified public HTTPS origin while authenticated customer calls used the published staging API and fixture state was fingerprinted directly in the staging PostgreSQL container. All five locale pages returned the exact allowlist with `no-store` and no seeded sensitive values; anonymous and non-owner cancellation returned `404` with an unchanged `Draft|xmin|updated_at` fingerprint; owner cancellation returned `200` and persisted `Cancelled`; lengths 24, 25, and 128 returned `404 + no-store`; and cleanup returned `0|0|0|0|0` for customers, reservations, auth sessions, background jobs, and audit logs. All five target containers remained healthy and PostgreSQL remained ready. A separate `notification-sms-send` job was already failing because Netgsm is not configured and did not match the controlled fixture; it is an operational notification issue, not WP2 residual data.
- 18 July follow-up revalidation on the same Dokploy development/staging VPS: the operator explicitly confirmed that this server is not production and remains in active development. SSH and Docker labels identified project `rent-a-car-zlatqa`; the deployed source remained `main` commit `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145` with only the operator-managed Compose file modified. The self-cleaning five-locale reservation/ownership matrix passed again and reported `VPS_RESIDUAL_COUNTS=0|0|0|0|0`. A separate fail-closed matrix returned `404` for all five localized claim pages, the registration page, both public membership proxies, and direct backend registration/claim paths including case and trailing-slash variants; the existing-customer login page remained `200`. Payment intent, forged 3DS return, and forged webhook probes each returned `503`. The before/after database fingerprint for customers, claim tokens, background jobs, audit logs, payment intents, and webhook events remained unchanged at `0|0|2|4|0|0`. Public-code lengths 24, 25, and 128 each returned `404` with `no-store, no-cache`. All five containers remained healthy and PostgreSQL accepted connections. A bounded secret-pattern scan returned zero matches. The API log contained one earlier PostgreSQL password-authentication failure and one restart during startup, but the API was healthy, all DB-backed acceptance flows passed, and the error did not recur in the final ten-minute window. Netgsm and SMS-job triage were explicitly excluded from this follow-up. This expands development/staging evidence only and does not close the separate final-production revalidation gate.
...
- Dependabot lifecycle evidence (20 July 2026): ruleset `18985047`, active since `2026-07-15T14:19:21.492+03:00`, governed Dependabot PR #422 from creation at `2026-07-20T06:20:15Z`. GitHub applied the `dependencies` and `backend` labels. The one-line tool-manifest change upgraded `dotnet-ef` from 10.0.9 to 10.0.10, preserving the EF Core 10 major-version alignment. The initial head passed its checks, but `main` advanced and the ruleset correctly left the PR `BEHIND`; the operator requested a Dependabot rebase instead of bypassing the current-branch requirement. Rebased head `e0dff18f083fac0a5ad8f4e9a92c6ad35f7c0df8` then passed backend unit/integration, frontend lint/test/build, Docker build, CodeQL for C# and JavaScript/TypeScript, Gitleaks, React Doctor, Snyk, and Semgrep. The final gate was `CLEAN`/`MERGEABLE` with zero review threads and the exact head unchanged. After the operator's explicit manual decision, the PR was squash-merged at `2026-07-20T20:28:58Z` as `134c6c888ff510c4eb1adfab1e41ebc0c5d83793`. Post-merge `main` CI, Secret Scan, React Doctor, and both CodeQL languages passed; CI also published API and Worker images to GHCR. That registry publication was not a production deployment. A live follow-up found zero open Dependabot alerts and ruleset `18985047` still active. This closes the tracked post-ruleset Dependabot operational-evidence gate; future dependency PRs remain subject to the same manual review and merge policy.
...
The focused final validation found no currently reachable original account-claim, public reservation disclosure/cancellation, production Mock, Dependabot auto-merge, or enabled-payment callback exploit path under the tracked configuration. The account-claim abuse-control/cleanup gates are locally proven, and the public reservation/cancellation boundary is now proven locally and on the Dokploy development/staging deployment at exact commit `0a5e7fa94bc94c6eb9ae986bc5bc2c57939dc145`. As of the 17 July 2026 product decision, public customer membership/account claim is outside the intended current product scope. The selected closure path is implemented and accepted locally and on the development/staging deployed public surface: PR #413 was merged as `fb7ca83e01599556ea9b06d24d9c570a4d0a111b`, post-merge CI/security and GHCR publication passed, and the operator-triggered Dokploy deployment passed cache-bypassed HTTP + Chromium checks for the five locale claim pages, registration page, public proxies, homepage link removal, and retained direct existing-customer login. Live proxy probes used empty JSON and mutated no staging data. The 18 July direct-VPS follow-up additionally proved the backend case/trailing-slash membership variants, unchanged membership/payment database fingerprint, and all three disabled-payment `503` responses on the same development/staging deployment. This closes the source/local/deployed-public development/staging membership-delivery alternative; it does not constitute final-production acceptance. Future automatic email is intended for reservation lifecycle notifications, but its provider and exact event matrix remain undecided. Neither the Resend-shaped nor Upstash-shaped scanner candidates requires rotation. The arbitrary-client-data-to-paid-state path remains contained while payments are disabled: local host-start and production-like Docker startup matrices prove the configured `ValidateOnStart` boundary, and the development/staging Dokploy public acceptance pass proves all three public payment entry points remain `503` after deployment. `IyzicoPaymentProvider.VerifyPaymentAsync` remains simulated and payments must not be enabled until provider-authenticated verification is implemented and proven. The Disabled-mode, public-membership development/staging deployed-public, WP2 development/staging, and tracked post-ruleset Dependabot operational-evidence blockers are closed. Final-production revalidation of the remaining original attack paths remains a separate gate. Real payment-provider proof remains an explicitly deferred product capability rather than evidence of current support.

### 18.3 External Session Handoff

The Dokploy payment-startup incident, explicit Disabled-provider implementation, verification evidence, publish workflow, and remaining operational gates are summarized in `C:\Users\muham\AppData\Local\Temp\2026-07-17-114631-dokploy-disabled-payment-fix-handoff.md`. The public-membership disable implementation, PR #413 publication, exact-commit Dokploy deployment, and public acceptance evidence are summarized in `C:\tmp\2026-07-17-153813-arac-kiralama-public-membership-disable-handoff.md`. The completed WP2 exact-commit Dokploy development/staging matrix, zero-residual cleanup, operational Netgsm warning, and next continuation steps are summarized in `C:\All_Project\Arac-Kiralama\docs\handoffs\2026-07-18-123633-wp2-dokploy-staging-acceptance.md`.

This document remains the durable closure authority. External handoffs are continuation aids and must not be committed; the repository handoff is an intentionally tracked continuation record and must remain aligned with this canonical status.
