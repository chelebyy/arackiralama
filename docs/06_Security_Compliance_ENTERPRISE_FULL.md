...
| --- | --- | --- | --- |
| Public customer membership | Registration and claim fail closed before controller/idempotency/persistence work; public pages and Next.js proxies return `404`; public header and customer-login page expose no registration entry point; retained hashed single-use claim controls remain defense in depth only | Fresh local validation passed 805/805 backend unit tests, 53/53 API integration tests, 296/296 frontend tests, production builds, and two Docker Chromium scenarios. Exact, case-variant, and trailing-slash registration/claim paths returned `404`; five localized claim pages and the direct register page returned `404`; the homepage exposed no customer login link; and customer/job fingerprints were unchanged | Source-level and local acceptance gate is closed. Merge/CI and deployed revalidation remain required. Future reservation-notification email is a separate undecided capability |
...
| Focused final validation | Instance-preserving revalidation of all seven original findings, followed by the 17 July public-membership closure implementation and scoped trust-boundary review | The earlier findings remain classified by their recorded evidence. The membership-disabled change passed full backend/frontend validation and Docker Chromium regressions; a trailing-slash bypass discovered during the scoped review was fixed and regression-tested. The Resend and Upstash scanner candidates remain not applicable; authoritative payment-provider verification remains deferred | Public membership is closed at source/local-acceptance level, not yet at deployed level. Deployed remaining attack-path revalidation and independent container evidence remain open, while real payment-provider proof remains deferred until payments are introduced |
