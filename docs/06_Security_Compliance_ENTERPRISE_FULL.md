...
| Dependency alert state | No current clean-scan claim; GitHub Dependabot is the live alert source; PR #405 merged patched Babel, Vite, esbuild, undici, and js-yaml versions | PR #405 passed the Node 22 frontend job, all repository checks, and exact-head Codex review, then squash-merged as `479317b`. The post-merge SBOM lists only patched target versions, but the Dependabot API still reports the same 11 open alerts after its update jobs completed. Local `pnpm audit` still returns registry HTTP `410`, so it is not a vulnerability result | Treat the alert set as unresolved until GitHub reconciles the 11 stale records to fixed, or investigate the platform state if they persist |
