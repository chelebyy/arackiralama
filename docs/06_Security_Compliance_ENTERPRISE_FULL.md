...
Date: 2026-02-25
Updated: 2026-07-16 (Dependabot remediation and alert-record reconciliation evidence added)
...
| Production payment configuration | `ValidateOnStart`; missing/Mock/unknown/sandbox/incomplete configurations rejected; a fully configured real provider may boot with payments disabled | Current host-start tests plus the fresh Release-image matrix pass: missing, Mock, unknown, sandbox, incomplete, and enabled Production configurations exit `139` with the general error and no synthetic secret leakage; the payments-disabled safe-shaped control returns `/health` `200` | GO for local configuration/startup acceptance; deployment rerun and real-provider sandbox proof remain separate gates |
...
| Secret artifacts | Generated Ship-Safe artifacts removed; scanner outputs ignored; Gitleaks scans working tree and Git history | Fresh CI-equivalent pinned Gitleaks working-tree/full-history scan passed; generated paths are untracked and ignored | Provider-side credential rotation, old-token rejection, and access-log review |
...
| Focused final validation | Instance-preserving revalidation of all seven original findings with the application-under-test at `202074f`; corrected committed account-claim harness re-run at `c860375` against the same Docker stack | Five candidates are suppressed by current dynamic/static counterevidence. Secret-provider incident evidence and authoritative payment-provider verification remain deferred. Full backend, full frontend, Docker build/health, two Chromium attack regressions, disabled-payment no-write proof, Gitleaks, and live GitHub governance checks passed | Production deployment rerun and provider credential/access-log evidence remain release blockers; payments remain disabled until real-provider proof |
