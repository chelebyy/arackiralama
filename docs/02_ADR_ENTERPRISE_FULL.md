...
These decisions are implemented for account claim, public reservation exposure/cancellation containment, explicit Disabled-provider payment containment, and production configuration validation. Account-claim abuse controls, retention cleanup, concurrency behavior, and five-locale Docker-browser behavior have local evidence. As of 17 July 2026, public customer membership/account claim is not intended to be part of the current product or release scope. However, the guest registration links and account-claim API path remain reachable in the current source, so this documentation-only product decision does not remove the capability or close its production-delivery gate. Before release, either a separate implementation must remove or disable the public registration/account-claim entry points, or a production email provider, trusted HTTPS claim-link configuration, controlled real delivery, and log-redaction proof remain mandatory. Future automatic email is intended instead for reservation lifecycle notifications; its provider and exact event matrix remain undecided.
...
The focused final validation completed on 16 July 2026 with the application-under-test at `202074f`; the corrected committed account-claim browser harness was then re-run from `9420446` against the same Docker acceptance stack. It found current counterevidence for the original guest-account claim, public reservation disclosure/cancellation, production Mock fallback, Dependabot auto-merge, and enabled callback attack paths. The account-claim browser contract intentionally carries the one-time token in the URL fragment and submits it through the same-origin `/api/auth/claim` proxy so the token is not placed in the query string. Payment requests and forged callbacks remain fail-closed with no database mutation while payments are disabled. The Resend and Upstash portions of the historical scanner incident are now triaged as not applicable for credential rotation: Resend had no project/provider anchor, while the three Upstash-shaped matches were arbitrary substrings inside Base64-encoded gzip .NET telemetry payloads and disappeared after decoding. Payment-integrity decisions, authoritative provider verification, mismatch/replay negatives, sandbox proof, deployed revalidation of the remaining original attack paths, and independent container evidence remain external or deferred gates. Account-claim production email acceptance stays a release gate while its public entry points remain reachable; disabling those entry points is the alternative closure path. Future reservation-notification email provider and event selection remain a separate product decision.
