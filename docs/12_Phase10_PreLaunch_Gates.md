...
| Credential incident closure | GO (TRIAGED) | CI-equivalent pinned Gitleaks working-tree/full-history scan passes and generated artifacts are untracked/ignored. The Resend-shaped candidate had no project/provider anchor. The three Upstash-shaped matches were arbitrary substrings inside Base64-encoded gzip .NET telemetry and disappeared after decoding. Neither candidate requires credential rotation or provider access-log review |
