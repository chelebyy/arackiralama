...
**Email delivery status:** No Resend adapter or credential is being introduced. The existing SMTP adapter remains disabled and unconfigured. The API production Compose mapping now passes `NEXT_PUBLIC_APP_URL` as `Notifications__PublicFrontendBaseUrl`, which is provider-independent and ensures future account-claim links use the configured HTTPS public origin.
...
**Durable repository controls:** The production Compose surface passes `NEXT_PUBLIC_APP_URL` to `Notifications__PublicFrontendBaseUrl`; SMTP logs no longer include recipient, subject, or raw provider exception text; generated `.dotnet/` telemetry is ignored.
