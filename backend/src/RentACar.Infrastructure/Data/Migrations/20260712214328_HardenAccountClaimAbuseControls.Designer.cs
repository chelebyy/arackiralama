// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using RentACar.Infrastructure.Data;

#nullable disable

namespace RentACar.Infrastructure.Data.Migrations
{
    [DbContext(typeof(RentACarDbContext))]
    [Migration("20260712214328_HardenAccountClaimAbuseControls")]
    partial class HardenAccountClaimAbuseControls
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("RentACar.Core.Entities.AdminUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("email");

                    b.Property<int>("FailedLoginCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("failed_login_count");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(140)
                        .HasColumnType("character varying(140)")
                        .HasColumnName("full_name");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<DateTime?>("LastLoginAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_login_at_utc");

                    b.Property<DateTime?>("LockoutEndUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("lockout_end_utc");

                    b.Property<string>("NormalizedEmail")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("normalized_email");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("password_hash");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("role");

                    b.Property<int>("TokenVersion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("token_version");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .HasDatabaseName("idx_admin_users_email");

                    b.HasIndex("NormalizedEmail")
                        .IsUnique()
                        .HasDatabaseName("ux_admin_users_normalized_email");

                    b.ToTable("admin_users", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.AuditLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("action");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Details")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("details");

                    b.Property<string>("EntityId")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("entity_id");

                    b.Property<string>("EntityType")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("entity_type");

                    b.Property<string>("IpAddress")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip_address");

                    b.Property<string>("NewValue")
                        .HasColumnType("jsonb")
                        .HasColumnName("new_value");

                    b.Property<string>("OldValue")
                        .HasColumnType("jsonb")
                        .HasColumnName("old_value");

                    b.Property<DateTime>("Timestamp")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("timestamp");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UserId")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.ToTable("audit_logs", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.AuthSession", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("CreatedByIp")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("created_by_ip");

                    b.Property<DateTime?>("LastSeenAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_seen_at_utc");

                    b.Property<Guid>("PrincipalId")
                        .HasColumnType("uuid")
                        .HasColumnName("principal_id");

                    b.Property<string>("PrincipalType")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("principal_type");

                    b.Property<DateTime>("RefreshTokenExpiresAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("refresh_token_expires_at_utc");

                    b.Property<string>("RefreshTokenHash")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("refresh_token_hash");

                    b.Property<Guid?>("ReplacedBySessionId")
                        .HasColumnType("uuid")
                        .HasColumnName("replaced_by_session_id");

                    b.Property<DateTime?>("RevokedAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("revoked_at_utc");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UserAgent")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("user_agent");

                    b.HasKey("Id");

                    b.HasIndex("RefreshTokenExpiresAtUtc")
                        .HasDatabaseName("idx_auth_sessions_refresh_token_expires_at_utc");

                    b.HasIndex("RefreshTokenHash")
                        .IsUnique()
                        .HasDatabaseName("ux_auth_sessions_refresh_token_hash");

                    b.HasIndex("RevokedAtUtc")
                        .HasDatabaseName("idx_auth_sessions_revoked_at_utc");

                    b.HasIndex("PrincipalType", "PrincipalId")
                        .HasDatabaseName("idx_auth_sessions_principal");

                    b.ToTable("auth_sessions", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.BackgroundJob", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime?>("FailedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("failed_at");

                    b.Property<string>("LastError")
                        .HasColumnType("text")
                        .HasColumnName("last_error");

                    b.Property<string>("Payload")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("payload");

                    b.Property<int>("RetryCount")
                        .HasColumnType("integer")
                        .HasColumnName("retry_count");

                    b.Property<DateTime>("ScheduledAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("scheduled_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("status");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("type");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("Type", "Payload")
                        .IsUnique()
                        .HasFilter("\"status\" IN ('Pending', 'Processing')");

                    b.ToTable("background_jobs", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.Campaign", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AllowedVehicleGroupIds")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("allowed_vehicle_group_ids");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("code");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("DiscountType")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("discount_type");

                    b.Property<decimal>("DiscountValue")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("discount_value");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<int>("MinDays")
                        .HasColumnType("integer")
                        .HasColumnName("min_days");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<DateOnly>("ValidFrom")
                        .HasColumnType("date")
                        .HasColumnName("valid_from");

                    b.Property<DateOnly>("ValidUntil")
                        .HasColumnType("date")
                        .HasColumnName("valid_until");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique();

                    b.ToTable("campaigns", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.Customer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateOnly?>("BirthDate")
                        .HasColumnType("date")
                        .HasColumnName("birth_date");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("email");

                    b.Property<int>("FailedLoginCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("failed_login_count");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(140)
                        .HasColumnType("character varying(140)")
                        .HasColumnName("full_name");

                    b.Property<string>("IdentityNumber")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("identity_number");

                    b.Property<DateTime?>("LastLoginAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_login_at_utc");

                    b.Property<int>("LicenseYear")
                        .HasColumnType("integer")
                        .HasColumnName("license_year");

                    b.Property<DateTime?>("LockoutEndUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("lockout_end_utc");

                    b.Property<string>("Nationality")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("nationality");

                    b.Property<string>("NormalizedEmail")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("normalized_email");

                    b.Property<string>("PasswordHash")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("password_hash");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("phone");

                    b.Property<int>("TokenVersion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("token_version");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .HasDatabaseName("idx_customers_email");

                    b.HasIndex("IdentityNumber")
                        .HasDatabaseName("idx_customers_identity_number");

                    b.HasIndex("NormalizedEmail")
                        .IsUnique()
                        .HasDatabaseName("ux_customers_normalized_email");

                    b.ToTable("customers", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.CustomerAccountClaimToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime?>("ConsumedAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("consumed_at_utc");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTime>("ExpiresAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at_utc");

                    b.Property<string>("IssuedFromIp")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("issued_from_ip");

                    b.Property<string>("IssuedUserAgent")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("issued_user_agent");

                    b.Property<DateTime?>("SupersededAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("superseded_at_utc");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("token_hash");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId")
                        .IsUnique()
                        .HasDatabaseName("ux_customer_account_claim_tokens_one_active")
                        .HasFilter("consumed_at_utc IS NULL AND superseded_at_utc IS NULL");

                    b.HasIndex("ExpiresAtUtc")
                        .HasDatabaseName("idx_customer_account_claim_tokens_expires_at_utc");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ux_customer_account_claim_tokens_token_hash");

                    b.HasIndex("CustomerId", "SupersededAtUtc", "ConsumedAtUtc")
                        .HasDatabaseName("idx_customer_account_claim_tokens_active_lookup");

                    b.ToTable("customer_account_claim_tokens", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.FeatureFlag", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("description");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("feature_flags", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333331"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Online ödeme seçeneklerini public rezervasyon akışında gösterir. Kapalıyken müşteriler ödeme yapmadan 24 saat stok bloklu talep oluşturur.",
                            Enabled = false,
                            Name = "EnableOnlinePayment",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333332"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Campaign and discount rules toggle",
                            Enabled = true,
                            Name = "EnableCampaigns",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333333"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "SMS bildirimlerinin gönderimini etkinleştirir",
                            Enabled = true,
                            Name = "EnableSmsNotifications",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333334"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Arabic (RTL) dil desteğini etkinleştirir",
                            Enabled = true,
                            Name = "EnableArabicLanguage",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("33333333-3333-3333-3333-333333333335"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Sistemi bakım moduna alır",
                            Enabled = false,
                            Name = "MaintenanceMode",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.Office", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)")
                        .HasColumnName("address");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("code");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("is_active");

                    b.Property<bool>("IsAirport")
                        .HasColumnType("boolean")
                        .HasColumnName("is_airport");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name");

                    b.Property<string>("OpeningHours")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("opening_hours");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("phone");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique();

                    b.ToTable("offices", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("11111111-1111-1111-1111-111111111111"),
                            Address = "Sekerhane Mah. Ataturk Blv. No:10 Alanya/Antalya",
                            Code = "ala",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsAirport = false,
                            Name = "Alanya Merkez",
                            OpeningHours = "08:00-22:00",
                            Phone = "+90 242 000 00 01",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("11111111-1111-1111-1111-111111111112"),
                            Address = "Gazipasa-Alanya Havalimani Terminal Ici",
                            Code = "gzp",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsAirport = true,
                            Name = "Gazipasa Airport",
                            OpeningHours = "24/7",
                            Phone = "+90 242 000 00 02",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.PasswordResetToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime?>("ConsumedAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("consumed_at_utc");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime>("ExpiresAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at_utc");

                    b.Property<Guid>("PrincipalId")
                        .HasColumnType("uuid")
                        .HasColumnName("principal_id");

                    b.Property<string>("PrincipalType")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("principal_type");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("token_hash");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("ConsumedAtUtc")
                        .HasDatabaseName("idx_password_reset_tokens_consumed_at_utc");

                    b.HasIndex("ExpiresAtUtc")
                        .HasDatabaseName("idx_password_reset_tokens_expires_at_utc");

                    b.HasIndex("TokenHash")
                        .IsUnique()
                        .HasDatabaseName("ux_password_reset_tokens_token_hash");

                    b.HasIndex("PrincipalType", "PrincipalId")
                        .HasDatabaseName("idx_password_reset_tokens_principal");

                    b.ToTable("password_reset_tokens", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.PaymentIntent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("amount");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("IdempotencyKey")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("idempotency_key");

                    b.Property<string>("Provider")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("provider");

                    b.Property<string>("ProviderIntentId")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("provider_intent_id");

                    b.Property<string>("ProviderTransactionId")
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("provider_transaction_id");

                    b.Property<string>("RefundIdempotencyKey")
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("refund_idempotency_key");

                    b.Property<string>("RefundReason")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("refund_reason");

                    b.Property<string>("RefundReferenceId")
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("refund_reference_id");

                    b.Property<decimal?>("RefundedAmount")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("refunded_amount");

                    b.Property<Guid>("ReservationId")
                        .HasColumnType("uuid")
                        .HasColumnName("reservation_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("status");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("ProviderIntentId")
                        .HasDatabaseName("idx_payment_provider_intent_id");

                    b.HasIndex("ProviderTransactionId")
                        .HasDatabaseName("idx_payment_provider_transaction_id");

                    b.HasIndex("Provider", "IdempotencyKey")
                        .IsUnique()
                        .HasDatabaseName("idx_payment_provider_idempotency");

                    b.HasIndex("ReservationId", "RefundIdempotencyKey")
                        .HasDatabaseName("idx_payment_reservation_refund_idempotency");

                    b.ToTable("payment_intents", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.PaymentWebhookEvent", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Payload")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("payload");

                    b.Property<bool>("Processed")
                        .HasColumnType("boolean")
                        .HasColumnName("processed");

                    b.Property<string>("ProviderEventId")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("provider_event_id");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("ProviderEventId")
                        .IsUnique()
                        .HasDatabaseName("idx_webhook_provider_event");

                    b.ToTable("payment_webhook_events", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.PricingRule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CalculationType")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("calculation_type");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<decimal>("DailyPrice")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("daily_price");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date")
                        .HasColumnName("end_date");

                    b.Property<decimal>("Multiplier")
                        .HasPrecision(8, 4)
                        .HasColumnType("numeric(8,4)")
                        .HasColumnName("multiplier");

                    b.Property<int>("Priority")
                        .HasColumnType("integer")
                        .HasColumnName("priority");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date")
                        .HasColumnName("start_date");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("VehicleGroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("vehicle_group_id");

                    b.Property<decimal>("WeekdayMultiplier")
                        .HasPrecision(8, 4)
                        .HasColumnType("numeric(8,4)")
                        .HasColumnName("weekday_multiplier");

                    b.Property<decimal>("WeekendMultiplier")
                        .HasPrecision(8, 4)
                        .HasColumnType("numeric(8,4)")
                        .HasColumnName("weekend_multiplier");

                    b.HasKey("Id");

                    b.HasIndex("StartDate", "EndDate")
                        .HasDatabaseName("idx_pricing_date_range");

                    b.HasIndex("VehicleGroupId", "Priority", "StartDate", "EndDate")
                        .HasDatabaseName("idx_pricing_group_priority_range");

                    b.ToTable("pricing_rules", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.PublicSiteSettings", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CompanyAddress")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("company_address");

                    b.Property<string>("CompanyEmail")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("company_email");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("company_name");

                    b.Property<string>("CompanyPhone")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("company_phone");

                    b.Property<string>("ContactPageChannelsJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("contact_page_channels_json");

                    b.Property<string>("ContactPageMapEmbedUrl")
                        .IsRequired()
                        .HasMaxLength(1200)
                        .HasColumnType("character varying(1200)")
                        .HasColumnName("contact_page_map_embed_url");

                    b.Property<bool>("ContactPageMapIsVisible")
                        .HasColumnType("boolean")
                        .HasColumnName("contact_page_map_is_visible");

                    b.Property<string>("ContactPageMapTitle")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("contact_page_map_title");

                    b.Property<string>("ContactPageOfficesJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("contact_page_offices_json");

                    b.Property<string>("ContactPageWorkingHoursJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("contact_page_working_hours_json");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("FooterBottomLinksJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("footer_bottom_links_json");

                    b.Property<string>("HeaderLinksJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("header_links_json");

                    b.Property<string>("HeroLinksJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("hero_links_json");

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("key");

                    b.Property<string>("PagesJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("pages_json");

                    b.Property<string>("QuickLinksJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("quick_links_json");

                    b.Property<string>("SocialLinksJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("social_links_json");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("WorkingHours")
                        .IsRequired()
                        .HasMaxLength(160)
                        .HasColumnType("character varying(160)")
                        .HasColumnName("working_hours");

                    b.HasKey("Id");

                    b.HasIndex("Key")
                        .IsUnique();

                    b.ToTable("public_site_settings", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.Reservation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("CustomerId")
                        .HasColumnType("uuid")
                        .HasColumnName("customer_id");

                    b.Property<DateTime?>("DriverDateOfBirth")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("driver_date_of_birth");

                    b.Property<string>("DriverFirstName")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("driver_first_name");

                    b.Property<string>("DriverLastName")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("driver_last_name");

                    b.Property<string>("DriverLicenseCountry")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("driver_license_country");

                    b.Property<DateTime?>("DriverLicenseExpiryDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("driver_license_expiry_date");

                    b.Property<DateTime?>("DriverLicenseIssueDate")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("driver_license_issue_date");

                    b.Property<string>("DriverLicenseNumber")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("driver_license_number");

                    b.Property<string>("Notes")
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("notes");

                    b.Property<DateTime>("PickupDateTime")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("pickup_datetime");

                    b.Property<Guid>("PickupOfficeId")
                        .HasColumnType("uuid")
                        .HasColumnName("pickup_office_id");

                    b.Property<string>("PricingSnapshot")
                        .HasColumnType("jsonb")
                        .HasColumnName("pricing_snapshot");

                    b.Property<string>("PublicCode")
                        .IsRequired()
                        .HasMaxLength(24)
                        .HasColumnType("character varying(24)")
                        .HasColumnName("public_code");

                    b.Property<Guid?>("QuoteId")
                        .HasColumnType("uuid")
                        .HasColumnName("quote_id");

                    b.Property<string>("QuoteReplayProof")
                        .HasColumnType("jsonb")
                        .HasColumnName("quote_replay_proof");

                    b.Property<DateTime>("ReturnDateTime")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("return_datetime");

                    b.Property<Guid>("ReturnOfficeId")
                        .HasColumnType("uuid")
                        .HasColumnName("return_office_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("status");

                    b.Property<decimal>("TotalAmount")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("total_amount");

                    b.Property<DateTime?>("UnpaidRequestExpiresAtUtc")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("unpaid_request_expires_at_utc");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("VehicleId")
                        .HasColumnType("uuid")
                        .HasColumnName("vehicle_id");

                    b.Property<uint>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.HasIndex("PickupOfficeId");

                    b.HasIndex("PublicCode")
                        .IsUnique();

                    b.HasIndex("QuoteId")
                        .IsUnique()
                        .HasFilter("quote_id IS NOT NULL");

                    b.HasIndex("ReturnOfficeId");

                    b.HasIndex("UnpaidRequestExpiresAtUtc")
                        .HasDatabaseName("idx_reservations_unpaid_request_expiry")
                        .HasFilter("status = 'UnpaidRequest' AND unpaid_request_expires_at_utc IS NOT NULL");

                    b.HasIndex("Status", "CreatedAt")
                        .HasDatabaseName("idx_reservations_status_created");

                    b.HasIndex("VehicleId", "PickupDateTime", "ReturnDateTime")
                        .HasDatabaseName("idx_reservations_active_dates")
                        .HasFilter("status IN ('Hold','UnpaidRequest','PendingPayment','Paid','Confirmed','Active')");

                    b.ToTable("reservations", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOption", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("code");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("IconKey")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("icon_key");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<bool>("IsArchived")
                        .HasColumnType("boolean")
                        .HasColumnName("is_archived");

                    b.Property<int>("MaxQuantity")
                        .HasColumnType("integer")
                        .HasColumnName("max_quantity");

                    b.Property<string>("PricingMode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("pricing_mode");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.Property<decimal>("UnitPrice")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("unit_price");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<uint>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique();

                    b.HasIndex("IsActive", "IsArchived", "SortOrder")
                        .HasDatabaseName("idx_reservation_extra_options_catalog");

                    b.ToTable("reservation_extra_options", null, t =>
                        {
                            t.HasCheckConstraint("ck_reservation_extra_options_icon_key", "icon_key IN ('baby', 'users', 'navigation', 'wifi')");

                            t.HasCheckConstraint("ck_reservation_extra_options_max_quantity", "max_quantity >= 1 AND max_quantity <= 20");

                            t.HasCheckConstraint("ck_reservation_extra_options_sort_order", "sort_order >= 0 AND sort_order <= 9999");

                            t.HasCheckConstraint("ck_reservation_extra_options_unit_price", "unit_price >= 0 AND unit_price <= 1000000");
                        });

                    b.HasData(
                        new
                        {
                            Id = new Guid("44444444-4444-4444-4444-444444444441"),
                            Code = "child_seat",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IconKey = "baby",
                            IsActive = false,
                            IsArchived = false,
                            MaxQuantity = 20,
                            PricingMode = "PER_DAY",
                            SortOrder = 10,
                            UnitPrice = 75m,
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Version = 0u
                        },
                        new
                        {
                            Id = new Guid("44444444-4444-4444-4444-444444444442"),
                            Code = "additional_driver",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IconKey = "users",
                            IsActive = false,
                            IsArchived = false,
                            MaxQuantity = 20,
                            PricingMode = "PER_RENTAL",
                            SortOrder = 20,
                            UnitPrice = 150m,
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Version = 0u
                        },
                        new
                        {
                            Id = new Guid("44444444-4444-4444-4444-444444444443"),
                            Code = "gps",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IconKey = "navigation",
                            IsActive = false,
                            IsArchived = false,
                            MaxQuantity = 1,
                            PricingMode = "PER_DAY",
                            SortOrder = 30,
                            UnitPrice = 8m,
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Version = 0u
                        },
                        new
                        {
                            Id = new Guid("44444444-4444-4444-4444-444444444444"),
                            Code = "wifi",
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IconKey = "wifi",
                            IsActive = false,
                            IsArchived = false,
                            MaxQuantity = 1,
                            PricingMode = "PER_DAY",
                            SortOrder = 40,
                            UnitPrice = 12m,
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            Version = 0u
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOptionTranslation", b =>
                {
                    b.Property<Guid>("OptionId")
                        .HasColumnType("uuid")
                        .HasColumnName("option_id");

                    b.Property<string>("Locale")
                        .HasMaxLength(5)
                        .HasColumnType("character varying(5)")
                        .HasColumnName("locale");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.HasKey("OptionId", "Locale");

                    b.HasIndex("Locale")
                        .HasDatabaseName("idx_reservation_extra_option_translations_locale");

                    b.ToTable("reservation_extra_option_translations", null, t =>
                        {
                            t.HasCheckConstraint("ck_reservation_extra_option_translations_locale", "locale IN ('tr', 'en', 'de', 'ru', 'ar')");
                        });

                    b.HasData(
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444441"),
                            Locale = "tr",
                            Description = "9-36 kg arası çocuklar için uygundur",
                            Name = "Çocuk Koltuğu"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444441"),
                            Locale = "en",
                            Description = "Suitable for children 9-36 kg",
                            Name = "Child Seat"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444441"),
                            Locale = "de",
                            Description = "Geeignet für Kinder 9-36 kg",
                            Name = "Kindersitz"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444441"),
                            Locale = "ru",
                            Description = "Подходит для детей 9-36 кг",
                            Name = "Детское кресло"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444441"),
                            Locale = "ar",
                            Description = "مناسب للأطفال 9-36 كجم",
                            Name = "مقعد أطفال"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444442"),
                            Locale = "tr",
                            Description = "Araçta ek sürücü de kullanabilir",
                            Name = "Ek Sürücü"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444442"),
                            Locale = "en",
                            Description = "An additional driver can also drive the vehicle",
                            Name = "Additional Driver"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444442"),
                            Locale = "de",
                            Description = "Ein zusätzlicher Fahrer kann das Fahrzeug auch fahren",
                            Name = "Zusätzlicher Fahrer"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444442"),
                            Locale = "ru",
                            Description = "Дополнительный водитель также может управлять автомобилем",
                            Name = "Дополнительный водитель"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444442"),
                            Locale = "ar",
                            Description = "يمكن لسائق إضافي قيادة السيارة أيضاً",
                            Name = "سائق إضافي"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444443"),
                            Locale = "tr",
                            Description = "Navigasyon cihazıyla asla kaybolmayın",
                            Name = "GPS Navigasyon"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444443"),
                            Locale = "en",
                            Description = "Never get lost with navigation device",
                            Name = "GPS Navigation"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444443"),
                            Locale = "de",
                            Description = "Mit Navigationsgerät nie verloren gehen",
                            Name = "GPS-Navigation"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444443"),
                            Locale = "ru",
                            Description = "Никогда не теряйтесь с навигационным устройством",
                            Name = "GPS-навигатор"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444443"),
                            Locale = "ar",
                            Description = "لا تضل الطريق مع جهاز الملاحة",
                            Name = "جهاز ملاحة GPS"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444444"),
                            Locale = "tr",
                            Description = "Araç içinde internetle bağlı kalın",
                            Name = "WiFi"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444444"),
                            Locale = "en",
                            Description = "Stay connected with in-car internet",
                            Name = "WiFi"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444444"),
                            Locale = "de",
                            Description = "Bleiben Sie mit Internet im Auto verbunden",
                            Name = "WiFi"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444444"),
                            Locale = "ru",
                            Description = "Оставайтесь на связи с интернетом в автомобиле",
                            Name = "WiFi"
                        },
                        new
                        {
                            OptionId = new Guid("44444444-4444-4444-4444-444444444444"),
                            Locale = "ar",
                            Description = "ابقَ متصلاً بالإنترنت داخل السيارة",
                            Name = "واي فاي"
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOptionVehicleGroup", b =>
                {
                    b.Property<Guid>("OptionId")
                        .HasColumnType("uuid")
                        .HasColumnName("option_id");

                    b.Property<Guid>("VehicleGroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("vehicle_group_id");

                    b.HasKey("OptionId", "VehicleGroupId");

                    b.HasIndex("VehicleGroupId")
                        .HasDatabaseName("idx_reservation_extra_option_vehicle_groups_group");

                    b.ToTable("reservation_extra_option_vehicle_groups", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationHold", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<Guid>("ReservationId")
                        .HasColumnType("uuid")
                        .HasColumnName("reservation_id");

                    b.Property<string>("SessionId")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("session_id");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<Guid>("VehicleId")
                        .HasColumnType("uuid")
                        .HasColumnName("vehicle_id");

                    b.HasKey("Id");

                    b.HasIndex("ExpiresAt")
                        .HasDatabaseName("idx_holds_expires");

                    b.HasIndex("ReservationId");

                    b.HasIndex("SessionId")
                        .HasDatabaseName("idx_holds_session");

                    b.HasIndex("VehicleId");

                    b.ToTable("reservation_holds", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationSelectedExtra", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("character varying(3)")
                        .HasColumnName("currency");

                    b.Property<string>("DescriptionSnapshot")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("description_snapshot");

                    b.Property<Guid>("ExtraOptionId")
                        .HasColumnType("uuid")
                        .HasColumnName("extra_option_id");

                    b.Property<string>("Locale")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("character varying(5)")
                        .HasColumnName("locale");

                    b.Property<string>("NameSnapshot")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name_snapshot");

                    b.Property<string>("OptionCodeSnapshot")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("option_code_snapshot");

                    b.Property<long>("OptionVersionSnapshot")
                        .HasColumnType("bigint")
                        .HasColumnName("option_version_snapshot");

                    b.Property<string>("PricingModeSnapshot")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("pricing_mode_snapshot");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer")
                        .HasColumnName("quantity");

                    b.Property<int>("RentalDaysSnapshot")
                        .HasColumnType("integer")
                        .HasColumnName("rental_days_snapshot");

                    b.Property<Guid>("ReservationId")
                        .HasColumnType("uuid")
                        .HasColumnName("reservation_id");

                    b.Property<decimal>("TotalPriceSnapshot")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("total_price_snapshot");

                    b.Property<decimal>("UnitPriceSnapshot")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("unit_price_snapshot");

                    b.HasKey("Id");

                    b.HasIndex("ExtraOptionId");

                    b.HasIndex("ReservationId")
                        .HasDatabaseName("idx_reservation_selected_extras_reservation");

                    b.HasIndex("ReservationId", "ExtraOptionId")
                        .IsUnique();

                    b.ToTable("reservation_selected_extras", null, t =>
                        {
                            t.HasCheckConstraint("ck_reservation_selected_extras_currency", "currency = 'TRY'");

                            t.HasCheckConstraint("ck_reservation_selected_extras_prices", "unit_price_snapshot >= 0 AND total_price_snapshot >= 0");

                            t.HasCheckConstraint("ck_reservation_selected_extras_quantity", "quantity >= 1 AND quantity <= 20");
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.Vehicle", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Brand")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("brand");

                    b.Property<string>("Color")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("color");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("GroupId")
                        .HasColumnType("uuid")
                        .HasColumnName("group_id");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("model");

                    b.Property<Guid>("OfficeId")
                        .HasColumnType("uuid")
                        .HasColumnName("office_id");

                    b.Property<string>("PhotoUrl")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("photo_url");

                    b.Property<string>("Plate")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("plate");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("character varying(40)")
                        .HasColumnName("status");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<int>("Year")
                        .HasColumnType("integer")
                        .HasColumnName("year");

                    b.HasKey("Id");

                    b.HasIndex("GroupId");

                    b.HasIndex("Plate")
                        .IsUnique();

                    b.HasIndex("OfficeId", "GroupId", "Status")
                        .HasDatabaseName("idx_vehicles_available")
                        .HasFilter("status = 'Available'");

                    b.HasIndex("OfficeId", "Status", "GroupId")
                        .HasDatabaseName("idx_vehicles_office_status_group");

                    b.ToTable("vehicles", (string)null);
                });

            modelBuilder.Entity("RentACar.Core.Entities.VehicleGroup", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<decimal>("DepositAmount")
                        .HasPrecision(18, 2)
                        .HasColumnType("numeric(18,2)")
                        .HasColumnName("deposit_amount");

                    b.Property<string>("Features")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("features");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("is_active");

                    b.Property<int>("MinAge")
                        .HasColumnType("integer")
                        .HasColumnName("min_age");

                    b.Property<int>("MinLicenseYears")
                        .HasColumnType("integer")
                        .HasColumnName("min_license_years");

                    b.Property<string>("NameAr")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name_ar");

                    b.Property<string>("NameDe")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name_de");

                    b.Property<string>("NameEn")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name_en");

                    b.Property<string>("NameRu")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name_ru");

                    b.Property<string>("NameTr")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("name_tr");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.ToTable("vehicle_groups", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("22222222-2222-2222-2222-222222222221"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            DepositAmount = 2000m,
                            Features = "[\"AirConditioning\",\"AutomaticTransmission\"]",
                            IsActive = true,
                            MinAge = 21,
                            MinLicenseYears = 2,
                            NameAr = "Economy",
                            NameDe = "Economy",
                            NameEn = "Economy",
                            NameRu = "Economy",
                            NameTr = "Ekonomi",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        },
                        new
                        {
                            Id = new Guid("22222222-2222-2222-2222-222222222222"),
                            CreatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            DepositAmount = 3500m,
                            Features = "[\"AirConditioning\",\"AutomaticTransmission\",\"BackupCamera\"]",
                            IsActive = true,
                            MinAge = 25,
                            MinLicenseYears = 3,
                            NameAr = "SUV",
                            NameDe = "SUV",
                            NameEn = "SUV",
                            NameRu = "SUV",
                            NameTr = "SUV",
                            UpdatedAt = new DateTime(2026, 3, 2, 0, 0, 0, 0, DateTimeKind.Utc)
                        });
                });

            modelBuilder.Entity("RentACar.Core.Entities.PaymentIntent", b =>
                {
                    b.HasOne("RentACar.Core.Entities.Reservation", "Reservation")
                        .WithMany("PaymentIntents")
                        .HasForeignKey("ReservationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Reservation");
                });

            modelBuilder.Entity("RentACar.Core.Entities.PricingRule", b =>
                {
                    b.HasOne("RentACar.Core.Entities.VehicleGroup", "VehicleGroup")
                        .WithMany("PricingRules")
                        .HasForeignKey("VehicleGroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("VehicleGroup");
                });

            modelBuilder.Entity("RentACar.Core.Entities.Reservation", b =>
                {
                    b.HasOne("RentACar.Core.Entities.Customer", "Customer")
                        .WithMany("Reservations")
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Office", "PickupOffice")
                        .WithMany()
                        .HasForeignKey("PickupOfficeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Office", "ReturnOffice")
                        .WithMany()
                        .HasForeignKey("ReturnOfficeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Vehicle", "Vehicle")
                        .WithMany()
                        .HasForeignKey("VehicleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Customer");

                    b.Navigation("PickupOffice");

                    b.Navigation("ReturnOffice");

                    b.Navigation("Vehicle");
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOptionTranslation", b =>
                {
                    b.HasOne("RentACar.Core.Entities.ReservationExtraOption", "Option")
                        .WithMany("Translations")
                        .HasForeignKey("OptionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Option");
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOptionVehicleGroup", b =>
                {
                    b.HasOne("RentACar.Core.Entities.ReservationExtraOption", "Option")
                        .WithMany("VehicleGroups")
                        .HasForeignKey("OptionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.VehicleGroup", "VehicleGroup")
                        .WithMany("ReservationExtraOptions")
                        .HasForeignKey("VehicleGroupId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Option");

                    b.Navigation("VehicleGroup");
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationHold", b =>
                {
                    b.HasOne("RentACar.Core.Entities.Reservation", "Reservation")
                        .WithMany("Holds")
                        .HasForeignKey("ReservationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Vehicle", "Vehicle")
                        .WithMany()
                        .HasForeignKey("VehicleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Reservation");

                    b.Navigation("Vehicle");
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationSelectedExtra", b =>
                {
                    b.HasOne("RentACar.Core.Entities.ReservationExtraOption", "ExtraOption")
                        .WithMany("SelectedExtras")
                        .HasForeignKey("ExtraOptionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Reservation", "Reservation")
                        .WithMany("SelectedExtras")
                        .HasForeignKey("ReservationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ExtraOption");

                    b.Navigation("Reservation");
                });

            modelBuilder.Entity("RentACar.Core.Entities.Vehicle", b =>
                {
                    b.HasOne("RentACar.Core.Entities.VehicleGroup", "Group")
                        .WithMany("Vehicles")
                        .HasForeignKey("GroupId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("RentACar.Core.Entities.Office", "Office")
                        .WithMany("Vehicles")
                        .HasForeignKey("OfficeId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Group");

                    b.Navigation("Office");
                });

            modelBuilder.Entity("RentACar.Core.Entities.Customer", b =>
                {
                    b.Navigation("Reservations");
                });

            modelBuilder.Entity("RentACar.Core.Entities.Office", b =>
                {
                    b.Navigation("Vehicles");
                });

            modelBuilder.Entity("RentACar.Core.Entities.Reservation", b =>
                {
                    b.Navigation("Holds");

                    b.Navigation("PaymentIntents");

                    b.Navigation("SelectedExtras");
                });

            modelBuilder.Entity("RentACar.Core.Entities.ReservationExtraOption", b =>
                {
                    b.Navigation("SelectedExtras");

                    b.Navigation("Translations");

                    b.Navigation("VehicleGroups");
                });

            modelBuilder.Entity("RentACar.Core.Entities.VehicleGroup", b =>
                {
                    b.Navigation("PricingRules");

                    b.Navigation("ReservationExtraOptions");

                    b.Navigation("Vehicles");
                });
#pragma warning restore 612, 618
        }
    }
}
